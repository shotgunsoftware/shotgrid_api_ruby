...
describe ShotgunApiRuby::Entities, :vcr do
  in_context 'with vcr values' do
    describe 'create' do
      subject(:create) { shotgun_client.assets.create(attributes) }
      ...
      context 'with a valid creation' do
        ...
        let(:attributes) do
          { description: description, project: { id: 155, type: :Project } }
        end
        ...
        it 'returns a valid entity' do
          ...
          expect(entity.relationships['project']['data']['id']).to eq(155)
          expect(entity.type).to eq('Asset')
        ensure
          shotgun_client.assets.delete(entity.id) if entity
        end

        it 'creates the entity' do
          previous_count = shotgun_client.assets.all.size
          entity = create
          next_count = shotgun_client.assets.all.size
    ...
    describe 'delete' do
      subject(:delete) { shotgun_client.assets.delete(id) }
      ...
      context 'when the entity exists' do
        let(:id) do
          entity =
            shotgun_client.assets.create(project: { type: 'Project', id: 155 })
          entity.id
        end
        ...
        it 'delete the entity' do
          expect { delete }.to change {
            shotgun_client.assets.first(filter: { id: id })
          }.to(nil)
        end
      end
    end
    ...
    describe 'update' do
      subject(:update) { shotgun_client.assets.update(id, changes) }
      ...
      context 'when the entity exists' do
        let(:id) do
          entity =
            shotgun_client.assets.create(
              { project: { type: :Project, id: 155 }, description: 'old' },
            )
          entity.id
        end

        after { shotgun_client.assets.delete(id) }
        ...
        context 'with changes' do
          ...
          it 'changes the entity' do
            expect { update }.to change {
              shotgun_client.assets.find(id).description
            }.from('old').to('new')
          end
    ...
    describe 'revive' do
      subject(:revive) { shotgun_client.assets.revive(id) }
      ...
      context 'when the entity exists' do
        ...
        before { shotgun_client.assets.delete(id) }
        ...
        it 'revive the entity' do
          expect { revive }.to change {
            shotgun_client.assets.first(filter: { id: id })&.description
          }.from(nil).to('old')
        end
      end
    end
    ...
    describe 'find' do
      subject(:find) do
        shotgun_client.assets.find(id, retired: retired, fields: fields)
      end
    ...
    describe 'first' do
      let(:shotgun_assets) { shotgun_client.assets }

      it 'forward to all' do
        expect(shotgun_assets).to receive(:all)
          .with(
            fields: :fields,
            sort: :sort,
            filter: :filter,
    ...
    describe 'all' do
      ...
      describe 'filter' do
        let(:filter) do
          first_asset = shotgun_client.assets.first
        ...
        context 'when the filters are too complex' do
          let(:shotgun_assets) { shotgun_client.assets }
          let(:filter) { { project: { id: 155 } } }
          it 'calls search instead' do
            expect(shotgun_assets).to receive(:search).and_call_original
            result = shotgun_assets.all(filter: filter)
            expect(
              result.map { |asset|
      ...
      describe 'page and page_size' do
        ...
        it 'returns the right items' do
          ...
          expect(all.first.id).to eq(
            shotgun_client.assets.all(sort: sort)[1].id,
          )
        end
      end
    ...
    describe 'search' do
      ...
      context "when there's an error" do
        let(:filter) { { project: { id: 155 }, not_existing_field: 'NOPE' } }
      ...
      describe 'filter' do
        let(:filter) do
          first_asset =
            shotgun_client.assets.first(filter: { project: { id: 155 } })
          { code: first_asset.code, project: { id: 155 } }
        end
        ...
        context 'when the filters are too simple' do
          ...
          it 'calls all instead' do
            expect(shotgun_assets).to receive(:all)
              .at_least(:once)
              .and_call_original
            result = shotgun_assets.search(filter: filter)
      ...
      describe 'page and page_size' do
        ...
        it 'returns the right items' do
          ...
          expect(search.first.id).to eq(
            shotgun_client.assets.search(sort: sort, filter: filter)[1].id,
          )
        end
      end
